...
pub struct Borrow<'info> {
    ...
    pub active_loan: Box<Account<'info, ActiveLoan>>,   /// have the data about the taken loan
    ...
    pub vault_asset_account: Account<'info, TokenAccount>,  /// hold the NFT

    /// CHECK: This is not dangerous
    #[account(mut)]
    pub vault_authority: AccountInfo<'info>,    /// will have authority over this vault NFT token account.
...
impl<'info> Borrow<'info> {
    /// transfer the NFT from the borrower_asset_account to the vault_asset_account
...
/// we take the minimum_balance_for_rent_exemption as a parameter. We take mutable references of active_loan, offer and collection and check if the loan is already taken or not. Then we set all the necessary values of active_loan.
pub fn handler(ctx: Context<Borrow>, minimum_balance_for_rent_exemption: u64) -> Result<()> {
    ...
    ///invoke token::transfer() to transfer the NFT from the borrower to the vault. Note here that this transfer is invoked from the anchor_spl whereas the transfer we were using before was system_program::transfer(). This is because when we have to transfer lamports, we would need to use system program, but if we need to transfer SPL Tokens, which include tokens and NFTs, then we need to use the token program.
    ...
    /// calculate the transfer amount that would go to the borrower.
    let transfer_amount = vault_lamports_initial
        .checked_sub(minimum_balance_for_rent_exemption)
        .unwrap();

    /// transfer lamports from the vault to the borrower.    
    **ctx
        .accounts
        .vault_account
        .to_account_info()
