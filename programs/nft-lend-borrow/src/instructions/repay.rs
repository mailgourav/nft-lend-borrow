...
///get a mutable reference of active_loan and change the is_repaid variable to true.
pub fn handler(ctx: Context<Repay>) -> Result<()> {
    ...
    ///when the borrower took the loan, the authority of the vault NFT token account was transferred to a certain vault authority. Well, now that the borrower is repaying, we need that vault authority to sign the transaction which will transfer the NFT from the vault back to the borrower.
    /// use Pubkey::find_program_address() to get the bump.
    let (_vault_authority, vault_auth_bump) =
        Pubkey::find_program_address(&[collection.key().as_ref()], ctx.program_id);
    ///derive the signer seeds using this bump and the collection key
    ...
    ///we use token::transfer() to transfer the NFT back to the lender by calling CpiContext::new_with_signer() and passing the signer seeds.
